...
void	send_to_all(int exception)
{
	...
	while (fd <= max_fd)
	{
		if (fd != exception && FD_ISSET(fd, &write_set))
...
int main(int argc, char **argv)
{
	...
	if (bind(server_fd, (const struct sockaddr *)&server_adress, sizeof(server_adress)) == -1
	...
	while (1)
	{
		...
		while (fd <= max_fd)
		{
			if (FD_ISSET(fd, &read_set))
			{
				if (fd == server_fd)
				{
					int client_fd = accept(server_fd, (struct sockaddr *)&server_adress, &len);
					if (client_fd == -1)
						write_error(NULL);
					if (client_fd > max_fd)
						max_fd = client_fd;
